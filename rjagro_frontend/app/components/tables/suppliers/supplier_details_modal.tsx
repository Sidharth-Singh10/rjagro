...
import { X, CreditCard, Receipt, Plus, ArrowLeft, Save, Banknote } from 'lucide-react';
import { useQueryClient } from '@tanstack/react-query'; // Import QueryClient
import { SupplierPayable, SupplierPayment } from '@/app/types/interfaces';
import { fetchSupplierPayables, fetchSupplierPayments, handleAddSupplierPayment } from '@/app/api/supplier';
...
export const SupplierDetailsModal: React.FC<SupplierDetailsModalProps> = ({ supplierId, isOpen, onClose }) => {
    ...
    const [viewMode, setViewMode] = useState<ViewMode>('list'); // New state for toggling views
    ...
    const refreshData = async () => {
        ...
        setLoading(true);
        try {
            const [payablesData, paymentsData] = await Promise.all([
    ...
    useEffect(() => {
        ...
        refreshData();
        setViewMode('list'); // Reset to list view when opening
    }, [supplierId, isOpen]);
                        ...
                        <div className="p-8 max-w-2xl mx-auto">
                            <PaymentForm
                                ...
                                onSuccess={() => {
                                    refreshData();
                                    setViewMode('list');
                                    setActiveTab('paid'); // Switch to paid tab to show new entry
                                }}
        ...
        <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
            ...
            <div className="mt-8 flex justify-end gap-3 pt-4 border-t">
                ...
                <button
                    type="submit"
                    disabled={loading}
                    className="inline-flex justify-center items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-md shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                    {loading ? (
                        <>Processing...</>
                    ) : (
