...
const UserSidebar: React.FC<UserSidebarProps> = ({ activeSection, onNavigate, isMobileOpen = false, onMobileClose }) => {
        ...
        return (
            <>
                ...
                <div className="h-16 flex items-center justify-between px-4 border-b border-gray-100 bg-white z-10">
                    <span className={`font-bold text-xl text-green-700 whitespace-nowrap overflow-hidden transition-all duration-300 ${!effectiveOpen && 'opacity-0 w-0'}`}>
                        RJ AGRO
                    </span>
                    {isMobile ? (
                        <button
                        ...
                        {navItems.map((item) => {
                            ...
                            return (
                                <li key={item.id} className="relative flex items-center">
                                    <button
                                        onClick={() => isMobile ? handleMobileNavigate(item.id) : onNavigate(item.id)}
                                        className={`
                                    ...
                                    >
                                        <div className={`
                                            ...
                                            ${!effectiveOpen && 'group-hover:bg-transparent'}
                                        `}>
                                            {item.icon ? (
                                                <item.icon
                                                    className={`
        ...
        <>
            ...
            <aside
                className={`
                    ...
                    transition-all duration-300 ease-in-out hidden md:flex flex-col relative z-20
                `}
            >
            ...
            {isMobileOpen && (
                <div className="fixed inset-0 z-40 md:hidden">
                    <div
                        ...
                        onClick={onMobileClose}
                    />
                    <aside className="relative z-50 w-72 h-full bg-white flex flex-col shadow-xl animate-[slideInLeft_0.2s_ease-out]">
                        {sidebarContent(true)}
                    </aside>
                </div>
            )}
